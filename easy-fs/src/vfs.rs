...
impl Inode {
    ...
    /// Return inode id of current inode.
    pub fn inode_id(&self) -> u32 {
        let fs = self.fs.lock();
        fs.get_disk_inode_id(self.block_id as u32, self.block_offset)
    }
    /// Return whether current inode is directory.
    pub fn is_dir(&self) -> bool {
        self.read_disk_inode(|disk_inode| disk_inode.is_dir())
    }
    /// Return hard link count of current inode.
    ...
    pub fn link(&self, old_name: &str, new_name: &str) -> isize {
        ...
        let mut fs = self.fs.lock();
        let Some(old_inode_id) = self.read_disk_inode(|root_inode| {
            assert!(root_inode.is_dir());
    ...
    pub fn unlink(&self, name: &str) -> isize {
        ...
        get_block_cache(inode_block_id as usize, Arc::clone(&self.block_device))
            .lock()
            .modify(inode_block_offset, |disk_inode: &mut DiskInode| {
                should_recycle = disk_inode.dec_nlink() == 0;
                if should_recycle {
